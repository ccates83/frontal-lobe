...
|------|----------|--------|---------|
| Agent | `~/.cursor/agents/{name}.md` | Markdown + YAML frontmatter | Autonomous task performers |
| Skill | `~/.cursor/skills/{name}/SKILL.md` | Markdown + YAML frontmatter | Reference patterns for agents |
| Command | `~/.cursor/commands/{name}.md` | Markdown + YAML frontmatter | User-invoked slash commands |
| Plugin | `~/.cursor/plugins/.../` | Directory with `.claude-plugin/plugin.json` | Bundled skills, agents, commands, hooks |
...
1. `~/.cursor/agents/` — list all existing agents
2. `~/.cursor/skills/` — list all existing skills
3. `~/.cursor/commands/` — list all existing commands
4. `~/.cursor/agents/frontal-lobe.md` — understand the orchestrator's routing table
