...
2. List existing agents in `~/.cursor/agents/`
3. Read `~/.cursor/agents/frontal-lobe.md` to understand the routing table
...
Delegate to the `meta-architect` subagent with:
...
- File paths to create under `~/.cursor/agents/`
- Existing agents in the same domain as style references

Then launch `meta-builder` again with:
- Instructions to update `~/.cursor/agents/frontal-lobe.md` routing table
...
1. Delegate to the `meta-builder` subagent with the specific fixes
