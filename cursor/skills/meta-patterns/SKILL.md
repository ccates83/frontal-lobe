...
Every new agent **MUST** be added to `~/.cursor/agents/frontal-lobe.md` in the Available Agents table. If the agent is not listed, frontal-lobe cannot invoke it.
...
| Claude Code | OpenCode |
|-------------|----------|
| `project guidance (CLAUDE.md or project guidance (CLAUDE.md or AGENTS.md))` | `project guidance (CLAUDE.md or AGENTS.md)` |
| `Cursor subagent` | `opencode` |
| `the appropriate tools` | `subagent delegation` |
| `~/.cursor/` | `~/.cursor/` |
| `cursor/` | `cursor/` |
| `` Delegate to the `agent-name` subagent `` | `` Use subagent delegation to invoke `@agent-name` `` |
...
```bash
scripts/convert.sh --to-opencode    # After editing cursor/ configs
scripts/convert.sh --to-claude      # After editing cursor/ configs
```
