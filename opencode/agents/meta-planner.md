...
|------|----------|--------|---------|
| Agent | `~/.config/opencode/agents/{name}.md` | Markdown + YAML frontmatter | Autonomous task performers |
| Skill | `~/.config/opencode/skills/{name}/SKILL.md` | Markdown + YAML frontmatter | Reference patterns for agents |
| Command | `~/.config/opencode/commands/{name}.md` | Markdown + YAML frontmatter | User-invoked slash commands |
| Plugin | `~/.config/opencode/plugins/.../` | Directory with `.claude-plugin/plugin.json` | Bundled skills, agents, commands, hooks |
...
1. `~/.config/opencode/agents/` — list all existing agents
2. `~/.config/opencode/skills/` — list all existing skills
3. `~/.config/opencode/commands/` — list all existing commands
4. `~/.config/opencode/agents/frontal-lobe.md` — understand the orchestrator's routing table
