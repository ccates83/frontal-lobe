...
argument-hint: "Agent description (e.g., 'Rust builder for embedded systems', 'Elixir planner and builder agents')"
---
...
2. List existing agents in `~/.claude/agents/`
3. Read `~/.claude/agents/frontal-lobe.md` to understand the routing table
...
- File paths to create under `~/.claude/agents/`
- Existing agents in the same domain as style references

Then launch `meta-builder` again with:
- Instructions to update `~/.claude/agents/frontal-lobe.md` routing table
...
1. Launch `meta-builder` with the specific fixes
