...
argument-hint: "Build target or flags (e.g., 'release', 'test', 'lint')"
---
...
1. Read CLAUDE.md, identify language and build tool
2. Read `go.mod` / `Cargo.toml` / `pom.xml` / `build.gradle.kts` / `*.csproj`
3. Determine build command

## Phase 2: Build
Launch `backend-builder` with build instructions. Fix errors (max 2 rounds).
